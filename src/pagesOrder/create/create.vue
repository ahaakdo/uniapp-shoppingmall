...
const getMemberOrderPreData = async () => {
  if (!query.orderId) {
    ...
    //获取再次购买订单
    const res = await getMemberOrderAgainAPI(query.orderId)
    orderPre.value = res.result
  }
}
onLoad(() => {
...
const selectedAddress = computed(() => {
  if (query.addressId) {
    return address.selectedAddress || address.buyNowAddress
  } else {
    return address.selectedAddress || orderPre.value?.userAddresses.find((item) => item.isDefault)
  }
})
//提交订单
const onOrderSubmit = async () => {
  if (!selectedAddress.value?.id) {
    return uni.showToast({ icon: 'fail', title: '请选择收获地址' })
  }
