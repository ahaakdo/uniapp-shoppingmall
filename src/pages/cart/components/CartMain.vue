...
//删除操作
const deleteMemberCartData = (skuId: string) => {
  uni.showModal({
    content: '是否删除？',
    success: async (res) => {
      if (res.confirm) {
  ...
  <scroll-view @scrolltolower="onScrolltolower" scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <template #right>
              <view class="cart-swipe-right">
                <button class="button delete-button" @tap="deleteMemberCartData(item.skuId)">
                  删除
                </button>
              </view>
            </template>
      ...
      <view
        class="toolbar"
        :style="{
          paddingBottom: current === 'pages/cart/cart2' ? safeAreaInsets?.bottom + 'px' : 0,
        }"
      >
